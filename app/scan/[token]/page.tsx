...
import { Camera, CheckCircle, AlertCircle, ArrowRight, Upload, Loader2 } from "lucide-react"
import Link from "next/link"
import { useParams } from "next/navigation"

// Mock function to get checkpoint data by token
...
export default function ScanTokenPage() {
  ...
  useEffect(() => {
    const fetchCheckpoint = async () => {
      try {
        const response = await fetch(`/api/checkpoints/${token}`)
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.executorName || !formData.status || !checkpointData) return
    ...
    try {
      ...
      const response = await fetch("/api/scans", {
        ...
        body: JSON.stringify({
          ...
          user_name: formData.executorName,
      ...
      if (response.ok) {
        ...
        console.error("Error submitting scan")
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="h-8 w-8 animate-spin mx-auto mb-4 text-samaya-navy" />
          <p className="text-muted-foreground">جاري تحميل بيانات النقطة...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardContent className="text-center py-8">
            ...
            <h2 className="text-xl font-bold samaya-navy mb-2">رمز غير صحيح</h2>
            <p className="text-muted-foreground mb-6">الرمز المطلوب غير موجود في النظام</p>
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardContent className="text-center py-8">
            ...
            <p className="text-muted-foreground mb-2">تم تسجيل المرور بنجاح في النظام</p>
            <div className="text-sm text-muted-foreground mb-6">
              {checkpointData.school} - {checkpointData.name_ar}
            </div>
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="text-center">
            <div className="text-xs text-muted-foreground mb-1">
              ...
              <ArrowRight className="inline h-3 w-3 mx-1" /> {checkpointData.name_ar}
            </div>
            <h1 className="text-lg font-bold samaya-navy">{checkpointData.school}</h1>
            <h2 className="text-base font-semibold">{checkpointData.name_ar}</h2>
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex items-center space-x-3 space-x-reverse p-3 border rounded-lg hover:bg-muted/50">
                    ...
                    <Label htmlFor="maintenance" className="flex-1 text-base cursor-pointer">
                      <div className="flex items-center space-x-2 space-x-reverse">
